    ...
    <div className="relative min-h-dvh overflow-hidden">
      ...
      {showProblemReport && (
        <div className="fixed inset-0 z-40">
          ...
          <form onSubmit={submitProblemReport} className="absolute inset-x-2 top-[max(4rem,calc(env(safe-area-inset-top)+3.5rem))] max-h-[calc(100dvh-4.5rem-env(safe-area-inset-top))] overflow-y-auto overscroll-contain rounded-2xl border border-white/70 bg-white p-4 shadow-[0_24px_70px_rgba(0,0,0,0.34)] sm:left-auto sm:right-4 sm:w-[min(92vw,24rem)] max-[640px]:left-1/2 max-[640px]:right-auto max-[640px]:top-[max(3.25rem,calc(env(safe-area-inset-top)+2.75rem))] max-[640px]:w-[min(calc(100vw-1rem),20rem)] max-[640px]:-translate-x-1/2 max-[640px]:rounded-lg max-[640px]:p-2.5">
            <div className="flex items-start justify-between gap-3 border-b border-slate-200 pb-3 max-[640px]:pb-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-navy max-[640px]:text-[10px] max-[640px]:tracking-[0.12em]">Report a Problem</p>
                <h2 className="mt-1 text-base font-bold text-navy-dark max-[640px]:mt-0.5 max-[640px]:text-sm">Login Issue</h2>
              </div>
              <button type="button" aria-label="Close" onClick={() => setShowProblemReport(false)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 hover:text-navy max-[640px]:h-9 max-[640px]:w-9">
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="mt-3 space-y-2.5 sm:mt-4 sm:space-y-3 max-[640px]:mt-2 max-[640px]:space-y-2">
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 max-[640px]:text-[10px]">Name</label>
                <input value={problemForm.reporterName} onChange={(event) => setProblemForm((current) => ({ ...current, reporterName: event.target.value }))} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-base focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/15 sm:text-sm max-[640px]:rounded-lg max-[640px]:px-2.5" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 max-[640px]:text-[10px]">Email</label>
                <input type="email" value={problemForm.reporterEmail} onChange={(event) => setProblemForm((current) => ({ ...current, reporterEmail: event.target.value }))} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-base focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/15 sm:text-sm max-[640px]:rounded-lg max-[640px]:px-2.5" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 max-[640px]:text-[10px]">Subject</label>
                <input value={problemForm.subject} onChange={(event) => setProblemForm((current) => ({ ...current, subject: event.target.value }))} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-base focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/15 sm:text-sm max-[640px]:rounded-lg max-[640px]:px-2.5" required />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500 max-[640px]:text-[10px]">Problem Details</label>
                <textarea value={problemForm.message} onChange={(event) => setProblemForm((current) => ({ ...current, message: event.target.value }))} rows={4} className="w-full resize-none rounded-xl border border-slate-300 bg-white px-3 py-2 text-base focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/15 sm:text-sm max-[640px]:rounded-lg max-[640px]:px-2.5" required />
              </div>
              ...
              <button type="submit" disabled={problemSubmitting} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-navy px-4 py-2.5 text-sm font-bold text-white hover:bg-navy-light disabled:cursor-not-allowed disabled:opacity-60 max-[640px]:rounded-lg max-[640px]:text-xs">
                {problemSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                {problemSubmitting ? "Loading" : "Submit Report"}
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="overflow-hidden rounded-[30px] border border-white/85 bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(248,246,247,0.98))] shadow-[0_30px_80px_rgba(0,0,0,0.36)] ring-1 ring-white/50 max-[640px]:rounded-[20px] max-[640px]:shadow-[0_18px_48px_rgba(0,0,0,0.32)]">
            <div className="relative overflow-hidden border-b border-slate-200/90 px-4 pb-4 pt-5 sm:px-7 max-[640px]:px-2.5 max-[640px]:pb-2 max-[640px]:pt-2.5">
              ...
              <div className="relative z-10 text-center text-white">
                ...
                <img src={loginLogo} alt={settings.institutionName || settings.systemName} className="mx-auto mt-2 h-auto w-28 object-contain sm:w-32 max-[640px]:mt-0.5 max-[640px]:w-16" />
              </div>
            </div>
                    ...
                    {roleSelection.roles.map((item) => (
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          ...
                          <span className="truncate text-sm font-bold max-[640px]:text-xs">{formatRoleLabel(item)}</span>
                        </span>
